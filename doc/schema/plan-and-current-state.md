...
| `noActivityTimeoutMinutes` | no | Override the global `noActivityTimeout` (default 30 min) for this sub-part's health monitoring. Planning agents may need longer; utility sub-parts may need shorter. See [Health Monitoring — Per-Sub-Part noActivityTimeout Override](../use-case/HealthMonitoring.md#per-sub-part-noactivitytimeout-override) (ref.ap.RJWVLgUGjO5zAwupNLhA0.E). |
