...
| Cross-try learning | **Ticket mutation via NonInteractiveAgentRunner** | On failure, run ClaudeCode `--print` (sonnet) via `NonInteractiveAgentRunner` (ref.ap.ad4vG4G2xMPiMHRreoYVr.E) to read `.ai_out/` artifacts and generate failure summary on stdout. The **harness** appends `## Previous Failed Attempts` section to the ticket, commits on the try branch, and performs best-effort propagation to the originating branch. Agent is read-only + stdout output — all file mutations and git operations are harness-owned. Ticket already feeds into agent context — no plumbing changes needed. See `TicketFailureLearningUseCase` (ref.ap.cI3odkAZACqDst82HtxKa.E). |
