...
**Simplification:** Return a `ProcessResult(val exitCode: Int, val output: String)` data class instead of a raw `Int`. This is simpler (one return type that captures everything) and callers can surface the output in error logs:
