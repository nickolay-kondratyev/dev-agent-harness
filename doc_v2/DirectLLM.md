# DirectLLM — Single Interface with Model Config (V2) / ap.hnbdrLkRtNSDFArDFd9I2.E

> **Deferred from V1.** DirectLLM has zero V1 consumers — title compression was replaced by
> deterministic slug truncation, and `LlmUserQuestionHandler` is a V2+ strategy
> (ref.ap.NE4puAzULta4xlOLh5kfD.E). Deferring eliminates: the DirectLLM interface,
> Anthropic-compatible API data classes, LLM service wiring, LLM credentials requirement,
> LLM API error handling, and the `glm` package internal data classes.
...
## V2 Model Assignments

| Use case | Model | Provider | Caller |
|---|---|---|---|
| Title compression, feature name suggestion | **GLM-4.7-Flash** | Z.AI (GLM) | *(unused after deterministic slug truncation — may be re-evaluated in V2)* |
...
Serialized via `Json.encodeToString(...)`. Single `user` message, no streaming.
