...
| `POST /callback-shepherd/signal/user-question` | Agent has a question for the human. Server returns 200 immediately; answer delivered asynchronously via TMUX `send-keys`. Duplicate questions (same session, same text) are deduplicated — the human sees each distinct question exactly once (ref.ap.Girgb4gaq2aecYTHjUj8a.E). |
